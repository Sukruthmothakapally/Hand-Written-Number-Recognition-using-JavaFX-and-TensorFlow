...
#save the model (so to prevent the model starting from start and training over and over again)
#model.save('handwritten_26_11.model')

#comment out above lines and load the model directly from saved location
#model = tf.keras.models.load_model('handwritten_26_11.model')
...
#get the real image from location

img=cv2.imread("C:\\Users\\Rakshith Shetty\\Documents\\project\\handwritten_num_reg_demo\\number.png")[:,:,0] #reading the image using computer vision cv2. we don't require color of image, hence getting only b/w [:,:,0]
img=np.invert(np.array([img])) #invert the image because by default it will be white on black and not black on white
